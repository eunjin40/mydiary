    ...
    <>
      ...
      <Main>
        ...
        <div>
          ...
          <ul>
            {list.map((elem, index) => {
              return (<li key={index} onClick={ () => {handleClickList(index)} }>
                <p>{elem.open ? "사적" : "공개"}</p>
                <p>{`${elem.year}년도 ${elem.month}월 ${elem.day}일 ${elem.date}요일`}</p>
                <p>{`제목: ${elem.title}`}</p>
                <p>{`메모: ${elem.memo}`}</p>
                <p>{`상세내용 : ${elem.detail.content}`}</p>
                <p> 그림 1 : <img src={elem.detail.pics[0]} alt={elem.detail.pics[0]} /></p>
                <p> 그림 2 : <img src={elem.detail.pics[1]} alt={elem.detail.pics[1]} /></p>
